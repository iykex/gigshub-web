...
export default function WalletPage() {
    ...
    const { data, isLoading } = useSWR(user ? `/api/user/wallet?userId=${user.id}` : null, fetcher)
            ...
            <div className="grid gap-6 md:grid-cols-2">
                ...
                <div className="grid gap-4">
                    <GlassCard className="flex flex-col justify-center items-center text-center space-y-2">
                        ...
                        <div className="text-2xl font-bold">GHS {transactions.filter((t: any) => t.type === 'topup').reduce((acc: number, t: any) => acc + t.change, 0).toFixed(2)}</div>
                    </GlassCard>
                    <GlassCard className="flex flex-col justify-center items-center text-center space-y-2">
                        ...
                        <div className="text-2xl font-bold">GHS {transactions.filter((t: any) => t.type === 'purchase').reduce((acc: number, t: any) => acc + t.change, 0).toFixed(2)}</div>
                    </GlassCard>
                </div>
            </div>
