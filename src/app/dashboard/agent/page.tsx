...
export default function AgentDashboard() {
  ...
  const { data: walletData } = useSWR<WalletResponse>(
  ...
  const { data: statsData } = useSWR<StatsResponse>(user ? `/api/user/stats?userId=${user.id}` : null, fetcher, {
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button variant="ghost" size="sm" asChild>
              <Link to="/dashboard/agent/orders">View all</Link>
            </Button>
          </div>
            ...
            {!statsData || !statsData.recentOrders || statsData.recentOrders.length === 0 ? (
              <div className="text-center text-muted-foreground py-12">
                ...
                <Button variant="link" asChild className="mt-2">
                  <Link to="/stores">Start processing orders</Link>
                </Button>
              </div>
            ) : (
