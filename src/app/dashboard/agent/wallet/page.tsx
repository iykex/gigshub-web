    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
                {/* Balance Card */}
                <div className="relative overflow-hidden rounded-xl bg-white/50 dark:bg-gray-900/50 p-8 text-white shadow-2xl">
                    ...
                    <div className="relative z-10 flex flex-col justify-between h-full space-y-8">
                        <div className="flex justify-between items-start">
                            <div>
                                <p className="text-gray-700 dark:text-gray-600 font-medium mb-1">Current Balance</p>
                        ...
                        <div className="flex gap-3">
                            <Link to="/wallet/topup" className="flex-1">
                                <Button variant="secondary" className="w-full bg-gray-700 dark:bg-gray-600 hover:bg-gray-600 dark:hover:bg-gray-700 text-white border-0 backdrop-blur-sm">
                                    <Plus className="mr-2 h-4 w-4" /> Top Up
                                </Button>
                            </Link>

                            <Button variant="secondary" className="flex-1 bg-gray-700 dark:bg-gray-600 hover:bg-gray-600 dark:hover:bg-gray-700 text-white border-0 backdrop-blur-sm">
                ...
                <div className="grid gap-4">
                    <GlassCard className="flex flex-col justify-center items-center text-center space-y-2">
                        ...
                        <div className="text-2xl font-bold">GHS {transactions.filter((t: Transaction) => t.type === 'topup').reduce((acc: number, t: Transaction) => acc + (t.change || 0), 0).toFixed(2)}</div>
                    </GlassCard>
                    <GlassCard className="flex flex-col justify-center items-center text-center space-y-2">
                        ...
                        <div className="text-2xl font-bold">GHS {transactions.filter((t: Transaction) => t.type === 'purchase').reduce((acc: number, t: Transaction) => acc + (t.change || 0), 0).toFixed(2)}</div>
                    </GlassCard>
                </div>
            </div>
                                    ...
                                    {transactions.map((t: Transaction) => (
                                        <tr key={t.id} className="hover:bg-muted/50">
                                            ...
                                            <td className={`px-6 py-4 font-medium ${(t.type === 'topup' || t.type === 'admin_credit') ? 'text-green-600' : 'text-red-600'}`}>
                                                {(t.type === 'topup' || t.type === 'admin_credit') ? '+' : '-'}GHS {(t.change || 0).toFixed(2)}
                                            </td>
                                            <td className="px-6 py-4">GHS {(t.balance_after || 0).toFixed(2)}</td>
