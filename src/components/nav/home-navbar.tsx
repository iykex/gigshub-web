...
import { Menu, X, ChevronRight, LogOut, User, LayoutDashboard, Wallet, ShoppingBag } from 'lucide-react'
...
function WalletButton({ isMobile = false, onClose }: { isMobile?: boolean, onClose?: () => void }) {
  const { user } = useAuth()
  const navigate = useNavigate()
      ...
      >
        ...
        >
          <span className="text-green-700 dark:text-green-400 flex items-center gap-2">
            ...
            Wallet
            <span className="ml-1 px-2 py-0.5 rounded-full text-xs font-bold bg-green-600 text-white">
  ...
  return (
    <Link to="/wallet/topup">
      <motion.div
        whileTap={{ scale: 0.95 }}
        className="relative px-3 py-2 rounded-full text-sm font-medium overflow-hidden group"
        style={{
          background: 'rgba(34, 197, 94, 0.1)',
          border: '1px solid rgba(34, 197, 94, 0.2)',
        }}
...
export function HomeNavbar() {
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 w-full px-4 sm:px-6 py-3">
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            >
              ...
              {/* Logo */}
              <Link to="/" className="flex items-center relative z-10 transition-transform hover:scale-105 active:scale-95">
                <img
